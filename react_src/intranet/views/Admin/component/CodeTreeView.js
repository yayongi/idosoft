...
export default function CodeTreeView(props) {
	...
	const { updateSelectedNodeId } = props;
	...
	function nestedRendered(item) {
		if(item.subTrees) {
			if(item.CODE_NAME == "ROOT"){
				return(
						<StyledTreeItem key={item.CODE_ID} nodeId={item.CODE_ID} label={item.CODE_NAME} onClick={() => handleNodeSelect("", "root")}>
							{
