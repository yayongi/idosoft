	...
	public void downloadExcelFile(@RequestBody Map<String, Object> params,
		...
		if(searchData != null) {
			...
			if("EXCEL0001".equals(fileCode) || "EXCEL0002".equals(fileCode)) {  
				...
				searchData.put("REG_START_DATE", REG_START_DATE);		// 그달의 시작일
				searchData.put("REG_END_DATE", REG_END_DATE);			// 그달의 종료일
				
			} else if("EXCEL0003".equals(fileCode) || "EXCEL0004".equals(fileCode)) {
				
				String expenseType 		= (String)searchData.get("expenseType");			// 경비 유형
				String payStDt 			= (String)searchData.get("payStDt");				// 시작 날짜
				String payEdDt 			= (String)searchData.get("payEdDt");				// 종료 날짜
				String status 			= (String)searchData.get("status");					// 결재 상태
